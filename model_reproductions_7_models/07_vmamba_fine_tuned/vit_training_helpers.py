...
# One shared 176/45 split for every experiment.
...
def read_common_split():
    ...
    for row in rows:
        ...
        if not path.is_file():
            ...
            path = Path.home() / "Desktop/Paper replication/data" / folder / row["file"]
        if not path.is_file():
...
def one_epoch(model, loader, loss_function, device, optimizer=None, last_stage=None, gradient_clip_norm=None):
        ...
        with torch.set_grad_enabled(training):
            ...
            if training:
                ...
                if gradient_clip_norm is not None:
                    torch.nn.utils.clip_grad_norm_([p for p in model.parameters() if p.requires_grad], gradient_clip_norm)
                optimizer.step()
...
def train_experiment(
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_accuracy = one_epoch(
            model, train_loader, loss_function, device, optimizer, last_stage if phase == "fine_tuned" else None
        )
...
def train_fine_tuned_stable(model, last_stage, output_dir, epochs=10, learning_rate=1e-5, batch_size=1, center_crop=True):
    """Notebook-friendly fine-tuning loop with explicit finite-value checks."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=False)
    # The small MPS fine-tuning path can produce non-finite gradients on this
    # machine. CPU is slower but gives stable, reproducible updates here.
    device = torch.device("cpu")
    configure_parameters(model, "fine_tuned", last_stage)
    model.to(device)
    train_loader, validation_loader = make_loaders(batch_size, center_crop)
    loss_function = nn.CrossEntropyLoss()
    trainable = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable, lr=learning_rate, weight_decay=1e-4)
    history, best_loss, best_accuracy, best_accuracy_loss = [], float("inf"), -1.0, float("inf")
    for epoch in range(1, epochs + 1):
        train_loss, train_accuracy = one_epoch(model, train_loader, loss_function, device, optimizer, last_stage, 1.0)
        validation_loss, validation_accuracy = one_epoch(model, validation_loader, loss_function, device)
        values = [train_loss, train_accuracy, validation_loss, validation_accuracy]
        if not all(np.isfinite(v) for v in values):
            raise FloatingPointError(f"Non-finite fine-tuning metric at epoch {epoch}: {values}")
        ...
        if validation_loss < best_loss:
            best_loss = validation_loss; torch.save(model.state_dict(), output_dir / "best_validation_loss.pth")
        if validation_accuracy > best_accuracy or (validation_accuracy == best_accuracy and validation_loss < best_accuracy_loss):
            best_accuracy = validation_accuracy; best_accuracy_loss = validation_loss
            torch.save(model.state_dict(), output_dir / "best_validation_accuracy.pth")
    torch.save(model.state_dict(), output_dir / "final_epoch.pth"); save_history(history, output_dir)
    summary = {}
    for name, filename in (("final", "final_epoch.pth"), ("best_loss", "best_validation_loss.pth"), ("best_accuracy", "best_validation_accuracy.pth")):
        model.load_state_dict(torch.load(output_dir / filename, map_location=device, weights_only=True))
        summary[name] = save_evaluation(name, model, validation_loader, loss_function, device, output_dir)
    (output_dir / "result_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (output_dir / "run_configuration.json").write_text(json.dumps({"phase": "fine_tuned_stable", "epochs": epochs, "batch_size": batch_size, "learning_rate": learning_rate, "weight_decay": 1e-4, "seed": 42, "training_images": 176, "validation_images": 45, "device": str(device)}, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2), flush=True)
...
def _extract_fixed_features(model, loader, device):
    ...
    with torch.inference_mode():
        for images, labels, _ in loader:
            all_features.append(model.forward_features(images.to(device)).cpu())
